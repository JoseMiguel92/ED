...
* Descripcion: Unidad para Listas Dinamicas con Enlace Doble *
* *
*******************************************************************}
...
BEGIN
	encontrado:=FALSE;
	pB:=l;
	IF NOT EsListaVacia(pB) THEN
	BEGIN
		WHILE (pB<>NIL) AND NOT encontrado DO
		BEGIN
			encontrado:=IgualesElem(e,pB^.info);
	...
	BEGIN
		IF pB^.ante=NIL THEN
			l:=l^.sgte
		ELSE IF pB^.sgte=NIL THEN
			pB^.ante^.sgte:=NIL
		ELSE
		BEGIN
			pB^.ante^.sgte:=pB^.sgte;
			pB^.sgte^.ante:=pB^.ante;
		END;{ELSE}
		dispose(pB);
	END;{IF}
END;{BorrarElemento}
